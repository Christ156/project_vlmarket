    ...
    <div class="container text-center">
        ...
        <div class="container row d-flex my-3">
            <div class="col-10">
                <form action="{{ route('search') }}" method="post">
                    ...
                    <div class="input-group">
                        <input type="text" class="form-control" placeholder="Cari produk lainnya?"
            ...
            @else
                @if (Auth::user()->rank == '1')
                    <div class="col-2">
                        <a name="" id="" class="btn btn-outline-danger mx-2" href="{{ 'market/create' }}"
                            role="button">Tambah Produk</a>
                    </div>
                @endif
            @endguest
        </div>
        ...
        <!-- Product list -->
                                    ...
                                    <div class="row d-flex">
                                        @guest
                                            <a href="{{ route('market.show', [$m->id]) }}"
                                                class="btn btn-danger bg-red-secondary col mx-1">Detail</a>
                                        @else
                                            <a href="{{ route('market.show', [$m->id]) }}"
                                                class="btn btn-danger col mx-1">Preview</a>
                                            @if (Auth::user()->email == 'christvaldo36@gmail.com')
                                                <a href="{{ route('market.edit', [$m->id]) }}"
                                                    class="btn btn-outline-secondary col mx-1">Ubah</a>
                                            @endif
                                        @endguest
                                    </div>
                                </div>
            ...
            @elseif (Auth::user()->status == '0')
                <div class="text-center p-5">
                    <p class="">
                        Mohon maaf, akun anda saat ini dinonaktifkan oleh administrator.
                    </p>
                </div>
            @endif
    </div>
